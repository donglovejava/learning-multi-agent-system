...
**约完成 25%。** 讯飞星火已接通，`POST /api/v1/resources` 一次请求并行生成 **5 种真实资源**（文档/练习题/思维导图/代码/拓展阅读），总耗时约 22s，HTTP 200。
剩下未通的：视频（依赖 SeeDance）、真实数据层（PostgreSQL/Neo4j/Milvus 仍是桩）、画像/评估 API、前端。
